...
float subXHeight(int i, float x, float y, float time) {
	float frequency = 2 * PI / wave[i].waveLength;
	float phase = wave[i].speed * frequency;
	float theta = dot(i, x, y);
	return Q * x * frequency * wave[i].amplitude * cos(theta * frequency + time * phase);
}
float xHeight(float x, float y, float time) {
	float height = 0.0f;
	for (int i = 0; i < numWaves; i++) {
		height += subXHeight(i, x, y, time);
	}
	return height;
}
float subYHeight(int i, float x, float y, float time) {
	float frequency = 2 * PI / wave[i].waveLength;
	float phase = wave[i].speed * frequency;
	float theta = dot(i, x, y);
	return Q * y * frequency * wave[i].amplitude * cos(theta * frequency + time * phase);
}
float yHeight(float x, float y, float time) {
	float height = 0.0f;
	for (int i = 0; i < numWaves; i++) {
		height += subYHeight(i, x, y, time);
	}
	return height;
}
...
float dWavedz(int i, float x, float y, float time) {
	...
	float phase = wave[i].speed * frequency;
	float theta = dot(i, x, y);
	float A = wave[i].amplitude * frequency / theta;
...
void render()
{
	...
	for (j = 0; j < RESOLUTION; j++)
	{
		...
		for (i = 0; i <= RESOLUTION; i++)
		{
			...
			x = i * delta - 1;
			surface[indice + 3] = x + xHeight(x,y,t);
			surface[indice + 4] = waveHeight(x, y, t);
			surface[indice + 5] = y+ yHeight(x,y,t);
			if (j != 0)
			{
