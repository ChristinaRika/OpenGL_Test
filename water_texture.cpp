...
struct waveSource {
	...
	float speed;
	float center[1][2];
	waveSource(float amplitude, float waveLength, float speed, float x, float y) {
...
waveSource wave(0.01f, 0.3f, -0.2f, 0.0f, 0.0f);
...
float dot(float x, float y) {
	float cx = x - wave.center[0][0];
	float cy = y - wave.center[0][1];
...
float waveHeight(float x, float y, float time) {
	...
	float theta = dot(x, y);
	return wave.amplitude * sin(theta * frequency + time * phase);
}
float dWavedx(float x, float y, float time) {
	float frequency = 2 * PI / wave.waveLength;
	float phase = wave.speed * frequency;
	float theta = dot(x, y);
	float A = wave.amplitude * x * frequency / theta;
...
float dWavedy(float x, float y, float time) {
	...
	float phase = wave.speed * frequency;
	float theta = dot(x, y);
	float A = wave.amplitude * y * frequency / theta;
	return A * cos(theta * frequency + time * phase);
}
